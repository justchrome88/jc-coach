> R02A2 canonical source: `_legacy_archive/r02a2-2026-07-11/docs/AI_COACH.md`. The original is preserved byte-identically; this copy updates canonical paths only.

Merged current facts from duplicate candidates (originals preserved):

- `_legacy_archive/r02a2-2026-07-11/docs/AI_COACH_PROVIDER_ARCHITECTURE.md`
- `_legacy_archive/r02a2-2026-07-11/docs/FEATURES_RU.md`

The provider boundary remains replaceable, but every provider consumes
structured, validated CS2 evidence and must fail closed on weak data. Runtime
AI persistence and user-facing feature claims are governed by current code,
tests, and the status/control plane; stale provider-next-step and live inventory
counts from the duplicate files are not current truth.
