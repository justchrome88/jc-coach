...
domains: `impact_leak` and `bad_fight_selection`. The next Product work is the
minimal functional two-mission UI, followed by the 30+10 replay. Package
version `0.1.0` remains intentionally independent.

## Repository map

- `app/` — application code: shallow API route composition, persistence under
  `app/db/`, bounded services under `app/services/`, and runtime contracts
  under `app/contracts/`.
...
The bounded service map and dependency rules are in
[app/services/README.md](app/services/README.md). Operational configuration
