...
- Steam import works as an alpha path with deterministic cursor semantics, repaired job truth/status taxonomy and repaired exact match-date truth, but still needs demo cleanup lifecycle, failed-demo quarantine/cleanup policy, durable scheduling, retry/backoff operations and a richer sync ledger before production/friends use.
