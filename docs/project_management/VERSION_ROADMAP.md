...
- Current status: WP-016 recommendation loop acceptance in progress after legacy refresh repair foundation.
...
| `v0.8` | Recommendation Loop Acceptance | `WP-016` | in progress | Accept recommendation -> next match -> evaluation -> progress as a coherent coach loop. WP-016B added legacy detection/refresh foundation; controlled production refresh is still pending. | `METRICS_GUARDIAN`, `UI_COACH_GUARDIAN`, `TEST_GUARDIAN` |
