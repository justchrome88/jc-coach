...
| `v0.9` | Real Data Onboarding / Bulk Demo Usage | `WP-017A`-`WP-017G` | in progress; WP-017A diagnosed, WP-017B planned, WP-017C completed no-new, WP-017C2 completed one-demo import/evaluation with batch-cap warning, WP-017D accepted with repair required, WP-017E repaired the auto-evaluation trigger | Onboard more real matches/demos through controlled one-demo batches with storage, DB, parser, recommendation and runtime evidence. WP-017C2 kept cap `1`, imported playable exact-date match `#75`, created recommendation evaluation `#77`, and left pending placeholder `#73`; WP-017F must validate the repaired automatic evaluation path on pending `#73` before promotion or any cap raise. | `PM_ORCHESTRATOR`, `IMPORT_GUARDIAN`, `DB_GUARDIAN`, `RUNTIME_GUARDIAN`, `TEST_GUARDIAN`, `METRICS_GUARDIAN` |
