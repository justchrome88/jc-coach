...
- Current status: v0.9 Real Data Onboarding / Bulk Demo Usage is in progress; WP-017A diagnosed the baseline, WP-017B planned the first controlled batch runbook, WP-017C completed one no-new controlled batch, and WP-017C2 imported/parsed/evaluated one new playable exact-date demo under cap `1` with a batch-cap warning.
...
| `v0.9` | Real Data Onboarding / Bulk Demo Usage | `WP-017A`-`WP-017F` | in progress; WP-017A diagnosed, WP-017B planned, WP-017C completed no-new, WP-017C2 completed one-demo import/evaluation with batch-cap warning | Onboard more real matches/demos through controlled one-demo batches with storage, DB, parser, recommendation and runtime evidence. WP-017C2 kept cap `1`, imported playable exact-date match `#75`, created recommendation evaluation `#77`, and left pending placeholder `#73`; WP-017D must review this evidence before any cap or continuation decision. | `PM_ORCHESTRATOR`, `IMPORT_GUARDIAN`, `DB_GUARDIAN`, `RUNTIME_GUARDIAN`, `TEST_GUARDIAN`, `METRICS_GUARDIAN` |
