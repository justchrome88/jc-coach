...
- Current status: v0.9 Real Data Onboarding / Bulk Demo Usage is in progress; WP-017A diagnosed the baseline, WP-017B planned the first controlled batch runbook, WP-017C completed one no-new controlled batch, WP-017C2 imported/parsed/evaluated one new playable exact-date demo under cap `1`, WP-017D required a repair for the automatic Steam-path evaluation trigger, WP-017E repaired that trigger in code/tests, WP-017F proved it on pending `#73`, WP-017G accepted post-batch data integrity with warnings, WP-017H accepted performance with warnings, WP-017I0 added root `AGENTS.md`, WP-017R repaired roadmap/WP registry governance, WP-017I diagnosed exact playlist mode as unrecoverable from persisted data, and WP-017J accepted explicit deferral for `v0.9`.
- Next active target: `v0.9`
- Next active WP: `WP-017K Real Data Onboarding Promotion to v0.9`
