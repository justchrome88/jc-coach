...
- Current status: v0.9 Real Data Onboarding / Bulk Demo Usage is in progress; WP-017A diagnosed the baseline, WP-017B planned the first controlled batch runbook, WP-017C completed one no-new controlled batch, WP-017C2 imported/parsed/evaluated one new playable exact-date demo under cap `1`, WP-017D required a repair for the automatic Steam-path evaluation trigger, WP-017E repaired that trigger in code/tests, WP-017F proved it on pending `#73`, WP-017G accepted post-batch data integrity with warnings, WP-017H accepted performance with warnings, WP-017I0 added root `AGENTS.md`, and WP-017R is repairing roadmap/WP registry governance.
- Next active target: `v0.9`
- Next active WP: `WP-017R Roadmap / WP Registry Governance Repair`
...
| `v0.9` | Real Data Onboarding / Bulk Demo Usage | `WP-017A`-`WP-017K` | in progress; WP-017A diagnosed, WP-017B planned, WP-017C completed no-new, WP-017C2 completed one-demo import/evaluation with batch-cap warning, WP-017D accepted with repair required, WP-017E repaired the auto-evaluation trigger, WP-017F proved it live on pending `#73`, WP-017G accepted data integrity with warnings, WP-017H accepted performance with warnings, WP-017I0 added root `AGENTS.md`, WP-017R is repairing registry governance | Onboard more real matches/demos through controlled one-demo batches with storage, DB, parser, recommendation and runtime evidence. WP-017G accepted 76 total matches, 22 playable demos, exact-date matches `#75/#76`, parser artifacts `#50/#51`, and recommendation `#5` progress `3/10`; WP-017H accepted sub-2s read-only helper performance with warnings. Numbering drift is now explicit: emergency repair WPs were inserted because the automatic evaluation trigger became a blocker, while match mode WPs remain open. Promotion is blocked until WP-017I/J are completed or explicitly deferred, then WP-017K may decide promotion. | `PM_ORCHESTRATOR`, `IMPORT_GUARDIAN`, `DB_GUARDIAN`, `RUNTIME_GUARDIAN`, `TEST_GUARDIAN`, `METRICS_GUARDIAN` |
