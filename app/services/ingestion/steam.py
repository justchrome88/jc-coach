"""Canonical Steam history, account, and sync integration."""
...
from app.services.ingestion import demo_downloader
...
from app.services.ingestion.settings import get_app_setting
from app.services.ingestion.share_code import decode_match_share_code, parse_share_code_input
from app.services.ingestion.storage_guard import SteamImportStorageBudget, SteamStorageBudgetExceeded
...
def run_steam_import_all_job(db: Session, job_id: int) -> dict[str, Any]:
    ...
    try:
        ...
        demo_download = demo_downloader.download_pending_steam_demos(
            db,
...
def import_steam_share_code_demo(db: Session, steam_account_id: int, share_code_input: str) -> dict[str, Any]:
    ...
    try:
        ...
        demo_download = demo_downloader.download_pending_steam_demos(
            db, limit=1, share_codes=[share_code], import_job_id=job.id
        )
        taxonomy = classify_steam_import_all_result(
            accounts_count=1,
            account_states=[
