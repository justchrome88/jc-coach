...
| Production deployment/manual beta release | completed_with_warnings | R04 backed up production, passed the full gate, restarted the real service once, verified the two-card read model, and made no Product-data mutation. |
| Live personal beta | released_manual_user_led | R05 begins when the user logs in and manually activates a mission; imports remain one match at a time. |
...
- [x] R04: production deployment and manual beta release passed with one verified
  backup, one healthy restart, two proposal-ready cards, and zero data mutation.
