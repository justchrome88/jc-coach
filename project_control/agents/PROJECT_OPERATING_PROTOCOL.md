...
`project_control/agents/AGENT_WORKFLOW.md` defines the repo-native role
...
output modes, control-plane protection policy, role invocation shortcuts, the
standard Task Card contract and WP-level console output contract. Role behavior
contracts live in `project_control/agents/roles/` as Warm role definitions.

## 3. Source Of Truth Hierarchy

1. `AGENTS.md` - Codex operating contract.
2. `project_control/status/CURRENT_STATUS.md` - current product, runtime and blocker snapshot.
3. `project_control/planning/WP_REGISTRY.md` - WP IDs, statuses,
   dependencies and report paths.
4. `project_control/status/HANDOFF.md` - new-session bootstrap.
5. `project_docs/architecture/DECISIONS.md` - durable decisions.
...
3. `project_control/planning/WP_REGISTRY.md`

New-session Hot context additionally includes `project_control/status/HANDOFF.md`.
...
|---|---|
| `project_control/status/CURRENT_STATUS.md` | Current WP, blockers, promotion status, runtime assumptions or accepted limitations change. |
| `project_control/planning/WP_REGISTRY.md` | A WP is created, activated, completed, blocked, deferred or superseded, or dependency/report path changes. |
| `project_control/status/HANDOFF.md` | At session handoff, after WP completion, before a long pause, or when the next safe step changes. |
| `project_docs/architecture/DECISIONS.md` | A durable process, product or architecture decision is made. |
| `project_control/checklists/MASTER_WP_CHECKLIST.md` | The high-level campaign plan changes or a WP is added, renamed or reordered. |
| `project_control/manifests/DOCS_MAP.md` | Document roles or context levels change. |
| `project_control/agents/AGENT_WORKFLOW.md` | WP role workflow, control-plane protection policy, invocation modes, output modes, task type profiles, Task Card contract, Documentation Steward triggers or closure gate rules change. |
| `project_control/agents/roles/*.md` | Workflow role behavior, role handoff expectations or role modification rules change. |
| `/opt/jc-coach-pm/reports/WP_*.md` | A WP-level task report is required. |
...
4. Add the item to `project_control/planning/WP_REGISTRY.md`.
5. Add or update `project_control/checklists/MASTER_WP_CHECKLIST.md`.
...
Use `project_control/agents/AGENT_WORKFLOW.md` for task type profiles,
control-plane protection policy, invocation modes, output modes, role shortcuts
...
`project_control/agents/AGENT_WORKFLOW.md`: schema scope, allowed
...
Long reports must be saved under `/opt/jc-coach-pm/reports/`. Console output should stay
...
- Per WP: run the Documentation Steward check from
  `project_control/agents/AGENT_WORKFLOW.md` before closure. This owns docs
