...
| commit authority | root/PM `AGENTS.md`; old workflow | root `AGENTS.md`; `project_control/agents/AGENT_WORKFLOW.md`; PM `AGENTS.md` | Stricter and mode-aware: manual task authority vs autonomous runner protocol; task wins. |
...
| current/next routing | Product Hot; PM state/indexes | `project_control/status/*`; `project_control/planning/WP_REGISTRY.md`; `project_control/planning/WORK_PACKAGE_BACKLOG.md`; PM fixed files/indexes | Stricter: R02A2C is accepted, no task is current, and released R02A3 → R03 → R04 is consistent everywhere. |
